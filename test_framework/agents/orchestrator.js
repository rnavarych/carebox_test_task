...
export class AgentOrchestrator {
  ...
  async runTests(options = {}) {
    ...
    try {
      ...
      // Phase 2.5: AI Test Generation (if test plan exists)
      ...
      if (testPlanResult?.testPlan) {
        ...
        try {
          generatedTests = await this.generateTestCases(testPlanResult.testPlan, templatesToTest);
          this.log(`  Generated ${generatedTests.testCases?.length || 0} test cases`);
        } catch (error) {
          this.log(`  Test generation failed: ${error.message}`);
          this.log(`  Falling back to static tests...`);
        }
      }

      // Phase 3: Dynamic Playwright Tests
      let playwrightResults = null;
      if (testPlanResult?.testPlan) {
        this.log('');
        this.log('Phase 3: Playwright Tests');
        this.log('─'.repeat(40));

        try {
          // Use dynamic runner if tests were generated, otherwise use static
          const useDynamic = generatedTests?.testCases?.length > 0;
          const pwResults = await this.runPlaywrightTests(useDynamic);
